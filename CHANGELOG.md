...
- `sync-from-upstream.yaml` — fail the `Detect chart directory` step with an explicit error when no chart directory can be resolved, instead of letting the following `yq` call die on `/Chart.yaml: no such file or directory`. A `chart_dir` input that points at a directory without `Chart.yaml` is reported with the passed value, instead of being told to pass the input.
