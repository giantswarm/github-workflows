...
- `sync-from-upstream.yaml` — a chart whose `.pre-commit-config.yaml` carries devctl's `helm-schema-<chart>` hook gets its `values.schema.json` from that hook's own pipeline instead of `shield-schema-gen`: its pinned `additional_dependencies` and its script up to the final diff, writing the result. The hook only verifies, so it could not repair schema-gen's output, and every sync PR of such a repository failed its own pre-commit check (the app-platform keys went missing). Repositories without the hook keep schema-gen.
- `fix-vulnerabilities.yaml` — set the PR title on every run, not only when the PR is created. The step force-pushes the branch and then skipped `gh pr create` for an already open PR, so a long-lived remediation PR kept the title it was opened with. PRs created before the Conventional Commits title landed stayed on `Remediate Nancy findings on <branch>` and failed `semantic-pull-request` forever, while their content was refreshed on every run.
...
- `sync-from-upstream.yaml` — validate the `chart_dir` input in the `Detect chart directory` step: a value that is absolute, contains empty, `.` or `..` components, or is not a directory with a `Chart.yaml` fails with an error that names the passed value and the expected form (`helm/myapp`). A trailing slash is stripped so the later `git show origin/main:<chart_dir>/...` comparisons and the `values-sync`/`schema-gen` container paths keep working.
- `update-chart.yaml` — new optional `chart_dir` input (e.g. `helm/myapp`) that selects the chart directly instead of resolving it from the repository name. The value is checked with the same rules as in `sync-from-upstream.yaml` and must be a directory under `helm/` that contains `Chart.yaml`. `sync-from-upstream.yaml` passes its own `chart_dir` through, so a repository whose chart is not named after it no longer fails in the `call-update-chart` job before the sync runs.
...
- `update-chart.yaml` — create the `automated-update` label before opening the PR, so `gh pr create` no longer fails in repositories where the label is missing.
