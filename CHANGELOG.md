...
- `sync-from-upstream.yaml` — the dependency-version detection step now falls back to a subchart's `version:` field when `appVersion:` is not set, instead of skipping the dependency.

### Changed

- `sync-from-upstream.yaml` — the automated PR title now names the updated dependencies instead of a static "automated update from upstream".
